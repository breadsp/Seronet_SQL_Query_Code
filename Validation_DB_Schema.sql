...
CREATE TABLE IF NOT EXISTS `seronetdb-Validated`.`Tube` (
  ...
  `Tube_Type_Expiration_Date` date,
  PRIMARY KEY (`Tube_ID`)
);
...
`Biospecimen_Collection_to_Test_Duration` float,
`Assay_Target_Organism` varchar(255),
`Assay_Target` varchar(255),
...
Primary Key(`Research_Participant_ID`(20), `Assay_ID`, `Assay_Target_Organism`(50), `Assay_Target`, `Measurand_Antibody`(50)),
FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Participant` (`Research_Participant_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (`Assay_ID`) REFERENCES Assay_Metadata (Assay_ID) ON DELETE CASCADE ON UPDATE CASCADE
...
Create table if not Exists `seronetdb-Validated`.`BSI_Reference_Panel_Aliquots` (
	`Biorepository_ID` varchar(255),
	`Subaliquot_ID` varchar(255) primary key,
...
drop table `seronetdb-Validated`.`Reference_Panel_Data`;
Create Table if not Exists `seronetdb-Validated`.`Reference_Panel_Data`(
	`Panel_Index` varchar(50) not null,
	`Participant ID` varchar(50),
	`Test_Result` varchar(50),
	`CBC` float,
	`Days Post Symptom Onset` varchar(50),
     primary key(`Panel_Index`, `Participant ID`)
);

drop table `seronetdb-Validated`.`Reference_Panel_Requests`;
Create Table if not Exists `seronetdb-Validated`.`Reference_Panel_Requests`(
	`Reference_Panel_ID` varchar(50),
    `Batch Number` int,
	`ISO Request #` varchar(50) not null,
	`SeroNet Y/N` varchar(50),
	`Institution` varchar(50),
	`# of Panels Requested` float,
	`Seronet Code` varchar(255),
     primary key(`Reference_Panel_ID`, `Batch Number`),
	 FOREIGN KEY (`Seronet Code`) REFERENCES CBC (`CBC_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
     FOREIGN KEY (`Reference_Panel_ID`) REFERENCES `Reference_Panel_Data` (`Panel_Index`) ON DELETE CASCADE ON UPDATE CASCADE
    );

Create Table if not Exists `seronetdb-Validated`.`Reference_Panel_Manifests`(    
	`Reference_Panel_ID` varchar(50),
	`Batch_ID` int,
	`Parent_Biorepository_ID` varchar(50),
	`Subaliquot_ID` varchar(50),
    `Destination_ID` varchar(255),
	`Batch Creation Date` date,
    `Box Number` int,
    `Row Number` int,
    `Column Number` int,
	`Comments` varchar(1000),
     primary key(`Reference_Panel_ID`, `Batch_ID`),
	 FOREIGN KEY (`Destination_ID`) REFERENCES CBC (`CBC_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
     FOREIGN KEY (`Reference_Panel_ID`) REFERENCES `Reference_Panel_Data` (`Panel_Index`) ON DELETE CASCADE ON UPDATE CASCADE
);

create table if not exists `seronetdb-Validated`.`Validation_Panel_Assays`(    
	`Organization` varchar(255),
	`Assay_ID` varchar(25),
	`Assay Type` varchar(100),
	`Assay Target` varchar(100),
	`Assay Target Sub Domain` varchar(100),
	`Antibody Isotype` varchar(50),
	`Assay Cutoff` varchar(255),
	`Assay Cutoff Units` varchar(255),
    primary key(`Organization`, `Assay_ID`, `Assay Target`,	`Assay Target Sub Domain`,	`Antibody Isotype`)
);
